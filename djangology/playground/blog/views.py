...
def character_list(request):
    ...
    return render(request, 'blog/character_list.html', {'character': characters})

def equipement_list(request):
    equipements = Equipement.objects.order_by('id_equip')
    occupants = Character.objects.filter(lieu="arene")
    occupants_names = ", ".join([o.id_character for o in occupants])
    message = f"Le lieu est occupé par {occupants_names}"
    return render(request, 'blog/equipement_list.html', {'equipement': equipements, 'message': message})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if nouveau_lieu.disponibilite == "libre":
                ...
                if nouveau_lieu.id_equip == "arene" and character.etat == "normal":
                    if randint(0, 1):
                ...
                print("nouveau_lieu : " + nouveau_lieu.id_equip)
                if nouveau_lieu.id_equip == "pokeball" and character.etat == "affame":
                    if randint(0, 1):
                        character.etat = "fatigue"
                    else:
                        character.etat = "blesse"
                ...
                elif nouveau_lieu.id_equip == "baie" and character.etat == "fatigue":
                    character.etat = "affame"
                    character.save()

                # si il est blessé et dans l'infirmerie
                elif nouveau_lieu.id_equip == "centre_pokemon" and character.etat == "blesse":
                    character.etat = "affame"
                    character.save()
                elif nouveau_lieu.id_equip == "pokeball" and character.etat == "affame":
